...
# Script produces figure 1 from the Chapelle Li 2011 article, 
# but for the prior mismatch that it mentioned in the paper, 
# but not illustrated.
...
# load the dataset
load(file = "../data/banditPriorMismatch.RData")
...
epsilon002_K10_PriorMismatch <- genPlot(
    plotData = filter(resultsSub, epsilon == "0.02", noArms == "10"),
    yMax = 900, yStep = 100
    )

epsilon002_K100_PriorMismatch <- genPlot(
...
filename <- paste0(outDir, "figure1_epsilon002_K10_PriorMismatch.pdf")
cairo_pdf(filename, height = 3, width = 3.5, onefile = TRUE)
print(epsilon002_K10_PriorMismatch)
dev.off()

filename <- paste0(outDir, "figure1_epsilon002_K100_PriorMismatch.pdf")
cairo_pdf(filename, height = 3, width = 3.5, onefile = TRUE)
print(epsilon002_K100_PriorMismatch)
dev.off()
