...
In practice, for scalability reasons, \textbf{policies} generally use a limited set of parameters $\theta_{t}$. This set of parameters summarizes all historical interactions \( H_{t'} = (x_{t,a_t},a_{t},r_{t,a_t}) \) over \emph{t}= \{1, \ldots, t'\}, ensuring that the dimensionality of $\theta_{t'} << H_{t'}$.
...
We set out to develop an implementation that stays close to the previous formalization but also offers an easily accessible programming interface. As a bonus, this kept the class structure of the package elegant and straightforward, with six classes forming the backbone of the package (see also Figure ~\ref{fig:cmab_chart}):
...
\begin{itemize}
         ...
         \item \code{Plot}: The R6 class \code{Plot} generates plots based on \code{History} data. It is usually invoked by calling the generic function \code{plot(h)}, where \code{h} is an \code{History} class instance.
\end{itemize}
...
In these lines, we start out by instantiating the \code{Policy} subclass \code{EpsilonGreedyPolicy} (covered in section 4.3) as object \code{policy}, with its parameter \code{epsilon} set to \code{0.1}. Next, we instantiate the \code{Bandit} subclass \code{SyntheticBandit} as \code{bandit}, with three Bernoulli arms, each offering a reward of one with probability $p$, and otherwise a reward of zero. For the current simulation, the \code{bandit}'s probability of reward is set to respectively 0.9, 0.1 and 0.1 per arm through its \code{weight} parameter. We then assign both our \code{bandit} and our \code{policy} to \code{Agent} instance \code{agent}. This \code{agent} is then added to a \code{Simulator} that is set to one hundred \code{simulations}, each with a \code{horizon} of one hundred---that is, \code{simulator} runs one hundred \code{simulations}, each with a different random seed, for one hundred time steps \code{t}.
...
On completion of all of its simulation runs, \code{Simulator} returns a \code{history} object that contains a complete log of all interactions, which can then, for example, be plotted or summarized:
...
Though \pkg{contextual} comes into its own when you start building your own bandit and policy classes, you can also run simulations with its built-in bandits and policies. See Table \ref{table:overview_policies} for an overview of the currently implemented policies and Table \ref{table:overview_bandits} for an overview of all currently implemented bandits.
...
To demonstrate how to run a basic simulation with \pkg{contextual}'s pre-implemented bandits and policies, we will compare two of contextual's pre-implemented polices in a scenario where each policy seeks to optimize the click-through rate of two different online ads over the course of 300 impressions.

The first policy is known as an Epsilon First policy or an A/B test. This policy will explore both ads at random for a preset period and from thereon only exploit the ad with the best click-through rate.

The other policy, known as an Epsilon Greedy policy \citep{Sutton1998e}, will explore one of the ads at random twenty percent of the time and exploits the ad with the best current click-through rate eighty percent of the time.
...
\end{kframe}
\end{knitrout}
As can be seen in the left plot in Figure \ref{fig:ctr_examples}, the Epsilon First policy is the clear winner in this particular scenario. We can, however, make some minor changes to the scenario and end up with a very different outcome. In the script below we set one ad to be more effective in one age group, and the other to be more effective in a second age group. Next, we add a contextual policy (LinUCB, \cite{Li2010}), and run the simulation:
...
As can be seen in the right plot in Figure \ref{fig:ctr_examples}, this time around, neither non-contextual policy (Epsilon Greedy nor Epsilon First) does better than chance, as on average, both arms are equally likely to produce a click. On the other hand, the "LinUCB" policy does very well, as this contextual policy is able to map its rewards to the observed contextual features. Of course, neither of the simulations in the current section is very realistic. One way to generate more realistic Bandit data would be to write a Bandit subclass with a more complex generative model. How to create such a bandit subclass is delineated in section 5. Another option would, for example, be to evaluate policies on an offline dataset---more on that in section 6.
...
Each of these bandits can be used to run policies without further ado. They can, however, also be used as superclasses for custom \code{Bandit} subclass implementations or as templates for \code{Bandit} implementation(s) that directly subclass \pkg{contextual}'s \code{Bandit} superclass.
...
Below, an example of Algorithm \ref{Alg:LiBandit} converted to pkg{contextual} BasicBandit subclass BasicLiBandit run on an offline simulation. The offline dataset used in this example contains the click-through data of an online web store. Each of its 350000 rows represents one page view, with click (1) or non-click (0) information plus contextual information on which out of four “persuasive strategies” was used to persuade visitors to click through on that particular page. See Figure \ref{fig:ctr_comparison} for the resulting click-through rate over time.
...
\begin{figure}[H]
  \centering
    \includegraphics[width=.99\textwidth]{fig/one_day}
      \caption{A plot of the cummulative reward rate (or click-through rate) for EGreedy, EGreedySeg, LinUCB Dis, LinUCB Hyb, UCB1 and UCB1Seg policies over eight days of events from the Yahoo dataset.}
      \label{fig:one_day}
\end{figure}
