...
\hlcom{#                                    +-----+----+-----------> ads: k = 3}
\hlcom{#                                    |     |    |}
\hlstd{click_probabilities} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}  \hlkwd{c}\hlstd{(}  \hlnum{0.2}\hlstd{,}  \hlnum{0.3}\hlstd{,} \hlnum{0.1}\hlstd{,}     \hlcom{# --> d1: old   (p=.5)}
                                    \hlnum{0.6}\hlstd{,}  \hlnum{0.1}\hlstd{,} \hlnum{0.1}   \hlstd{),} \hlcom{# --> d2: young (p=.5)}
                                                        \hlcom{#     features: d = 2}
...
\hlcom{      # Initialize Bandit. Set self$d and self$k here.}
    \},
    ...
...
\hlstd{EpsilonFirstPolicy} \hlkwb{<-} \hlstd{R6}\hlopt{::}\hlkwd{R6Class}\hlstd{(}
  \hlkwc{public} \hlstd{=} \hlkwd{list}\hlstd{(}
    ...
    \hlkwc{set_parameters} \hlstd{=} \hlkwa{function}\hlstd{() \{}
      ...
      \hlcom{# Here, we define a list with 'n' and 'mean' theta parameters to each arm}
      \hlcom{# through helper variable self$theta_to_arms. That is, when the number of}
      \hlcom{# arms is 'k', the above would equal:}

      \hlcom{# self$theta <- list(n = rep(list(0,k)), 'mean' = rep(list(0,k)))}

      \hlcom{# ... which would also work just fine, but is much less concise.}

      \hlcom{# When assigning both to self$theta directly and via self$theta_to_arms,}
      \hlcom{# do make sure to do it in that order.}
  ...
  \hlkwc{public} \hlstd{=} \hlkwd{list}\hlstd{(}
    ...
    \hlkwc{get_context} \hlstd{=} \hlkwa{function}\hlstd{(}\hlkwc{index}\hlstd{) \{}
      \hlstd{contextlist} \hlkwb{<-} \hlkwd{list}\hlstd{(}
        ...
        \hlkwc{X} \hlstd{=} \hlkwd{matrix}\hlstd{(private}\hlopt{$}\hlstd{S}\hlopt{$}\hlstd{day_part, self}\hlopt{$}\hlstd{d, self}\hlopt{$}\hlstd{k)}
      \hlstd{)}
      \hlstd{contextlist}
    \hlstd{\},}
...
\hlstd{website_data}   \hlkwb{<-} \hlkwd{getURL}\hlstd{(}\hlkwd{setDT}\hlstd{(}\hlkwd{read.csv}\hlstd{(}\hlkwd{textConnection}\hlstd{(website_data))))}
...
\hlstd{agents}         \hlkwb{<-} \hlkwd{list}\hlstd{(Agent}\hlopt{$}\hlkwd{new}\hlstd{(LinUCBHybridPolicy}\hlopt{$}\hlkwd{new}\hlstd{(}\hlnum{0.6}\hlstd{), bandit))}
