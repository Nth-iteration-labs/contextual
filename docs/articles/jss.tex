...
Before continuing with a formalization of contextual Multi-Armed Bandit problems, it may be useful to locate MAB in the greater area of machine learning. Generally, machine learning is divided into three main areas: supervised learning, unsupervised learning and reinforcement learning \citep{Li2017}. Each can be differentiated in how data is presented to the learning algorithm:

\begin{itemize}
\item Unsupervised learing (such as clustering) makes no use of labels or rewards but searches for structure in its input data.
\item Supervised learning (such as classification and regression) maps inputs to an outputs by training on pre-labeled example data.
\item Reinforcement learning is by trial and error - that is, in contrast to supervised learning, it only receives partial feedback: one reward for every sequential choice.
\end{itemize}

Within this division, bandit problems are to be found within reinforcement learning.  focusses on algorithms hatlearn by trial and error, but where context and state do not feed back on previous actions \cite{Agarwal2014}.
...
Though \pkg{contextual} really comes into its own when you start building your own bandit and policy classes, you can also use one of the package's build-in policies, and run them on one of \pkg{contextual}'s synthetic or offline bandits (see Table \ref{table:overview_policies} for an overview of available policies).

\begin{table}[H]
\begin{tabular}{@{}llllll@{}}
\toprule & \textbf{$\epsilon$-Greedy} & \textbf{UCB} & \textbf{Thomspon Sampling} & \textbf{Other} & \textbf{Special} \\ \midrule
MAB & \begin{tabular}[c]{@{}l@{}}$\epsilon$-Greedy\\ $\epsilon$-First \end{tabular} & \begin{tabular}[c]{@{}l@{}}UCB1 \\ UCB-tuned \end{tabular} & \begin{tabular}[c]{@{}l@{}}Thompson Sampling \\ BootstrapTS \end{tabular} & \begin{tabular}[c]{@{}l@{}}Softmax\\ Gittins\end{tabular} & \multirow{2}{*}{\begin{tabular}[c]{@{}l@{}}Random\\ Oracle\\ LiF\end{tabular}} \\ \cmidrule(r){1-5}
cMAB & Epoch-Greedy & \begin{tabular}[c]{@{}l@{}}LinUCB \\ COFIBA \end{tabular} & \begin{tabular}[c]{@{}l@{}}LinTS \\ LogitBTS \end{tabular} & & \\ \bottomrule
\end{tabular}
\caption{An overview of \pkg{contextual}'s predefined policy classes.}
\label{table:overview_policies}
\end{table}

You may, for instance, want to compare bandit policies for selecting the most effective of two ads for your new web store. One option would, for example, be to show both ads at random (explore) for a set period of time and thereafter go with the ad that has the best Click-Through Rate (exploit). This policy is known as an Epsilon First policy or, more commonly, as an A/B test. Another option might be to randomly show either one of the ads 20 percent of the time (explore), and show the best ad 80 percent of the time (exploit). This type of policy is known as an "Epsilon Greedy" policy. In other words, an Epsilon First policy starts out with a full-time exploration, then switches to full-time exploitation---while an Epsilon Greedy policy keeps on exploring. What policy does best in this simple case?

Let's see how to answer this question with \pkg{contextual}:
...
\end{figure}
In this second simulation, neither non-contextual policy (Epsilon Greedy and Epsilon First) does better than chance. That makes sense, as on average, both arms are equally likely to produce a click. On the other hand, contextual policies such as LinUCB do really well here, as these are able to map rewards to contextual features.

Of course, the simulations in the current section are not very realistic. One way to generate more realistic Bandit data would be to write a Bandit subclass with a more complex generative model that fits your particular use case. Another option would be to test policies against an offline data set. Either way, the next sections should be able to get you on your way there.
