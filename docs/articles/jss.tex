...
To demonstrate how to run a basic bandit simulation with contextual's pre-implemented bandits and policies, imagine a web store that seeks to evaluate two policies with the goal of selecting the policy that is best at evaluating which of two adds has the highest click-through rate.

The first policy explores both adds at random for a preset period (here, for instance for fifty steps) and from thereon only exploits the ad with the best click-through rate---a policy known as an Epsilon First policy or an A/B test.

The other policy explores one of the ads at random for 20 percent of the time and exploits the ad with the best current click-through rate 80 percent of the time---a type of policy known as "Epsilon Greedy" \citep{Sutton1998e}.
...
\end{kframe}
\end{knitrout}
As can be seen in the left plot in Figure \ref{fig:ctr_examples}, in this scenario, with two clearly differentiable stable arms, Epsilon First is the clear winner. Which makes sense: in contrast to the Epsilon Greedy policy, the Epsilon First policy does not incur any more exploration penalties from choosing the lesser arm from the moment it has picked the best arm.

We could, however, change this scenario just a little for a very different outcome. Let us, for instance, suggest that half the visitors to the web store are younger than forty, the other half older than forty. And that one of the ads is more effective for the younger visitors, while the other ad works better for the older ones.

We can simulate this scenario by making some minor changes to the previous script. Specifically, we adapt the Bandit by defining two contexts, with different reward probabilities per arm---and add a contextual policy (LinUCB, \cite{Li2010}):
...
As can be seen in the right plot in Figure \ref{fig:ctr_examples}, this time around, neither non-contextual policy (Epsilon Greedy nor Epsilon First) does better than chance, as on average, both arms are equally likely to produce a click. On the other hand, the contextual "LinUCB" policy does well, as it can map rewards to contextual features.
