...
\begin{itemize}
         \item \code{Bandit}: The R6 class \code{Bandit} is the parent class of all \code{Bandits} implemented in \code{\{contextual\}}. Classes that extend the abstract superclass \code{Bandit} are responsible for the generation of \code{context} vectors and \code{rewards} awarded to \code{actions} that are decided upon by its related \code{Policy} at each time step \code{t}. \code{Bandit} subclasses may (pre)generate these values synthetically, based on offline data, etc.

         \item \code{Policy}: The R6 class \code{Policy} is the parent class of all \code{Policies} implemented in \code{\{contextual\}}. Classes that extend this abstract \code{Policy} superclass superclass are expected to take into account the current \code{context}, together with a limited set of parameters denoted \code{theta} (summarizing all past actions), to chose one of a \code{Bandit}'s arms at each time step \code{t}. On receiving a \code{reward}, the \code{Policy} then uses the current \code{context}, \code{action} and \code{reward} to update its set of parameters \code{theta}.

         \item \code{Agent}: The R6 class \code{Agent} takes care of the flow of information between and the running of one \code{Bandit}/\code{Policy} pair.

         \item \code{Simulation}: The R6 class \code{Simulation} is the entry point of any \pkg{contextual} simulation. It encapsulates one or more \code{Agents}, clones them if necessary, runs the \code{Agents}, and saves the log of all of the \code{Agents} interactions to a \code{History} object.

         \item \code{History}: encapsulates an R \code{data.table}, keeps a log of all interactions, and provides data summaries.

         \item \code{Plot}: The R6 class \code{Plot} generates plots based on \code{History} data. It can also be invoked by calling the generic \code{plot(h)}, where \code{h} is an \code{History} class instance.
\end{itemize}

From this follows that in our framework, on being run by the Simulator, an Agent repeatedly performs the following operations at every \textit{t} in \textit{t}=1,2,{\dots},\textit{T} (see also Figure ~\ref{fig:cmab_chart})
