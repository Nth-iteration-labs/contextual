...
The current section offers an overview of \pkg{contextual}'s pre-implemented bandits and policies and explains how to run a basic simulation.

\subsection{Implemented Policies and Bandits}

Though \pkg{contextual} comes into its own when you start building your own bandit and policy classes, it is also possible to run simulations that make use of its built-in bandits and policies. See Table \ref{table:overview_policies} for an overview of all implemented policies. See Table \ref{table:overview_bandits} for an overview of all currently implemented bandits.
...
BasicBandit & \begin{tabular}[c]{@{}l@{}}BasicContextualBandit\\ MixedContextualBandit\end{tabular} & \begin{tabular}[c]{@{}l@{}}LiSamplingOfflineBandit\\ DoublyRobustOfflineBandit\end{tabular} & ContinuumBandit \\ \bottomrule
\end{tabularx}
...
To demonstrate how to run a basic simulation with \pkg{contextual}'s pre-implemented bandits and policies, imagine a web store that seeks to evaluate two policies with the goal of selecting the policy that is best at evaluating which of two adds has the highest click-through rate.

The first policy explores both adds at random for a preset period and from thereon only exploits the ad with the best click-through rate---a policy known as an Epsilon First policy or an A/B test.

The other policy explores one of the ads at random twenty percent of the time and exploits the ad with the best current click-through rate eighty percent of the time---a type of policy known as "Epsilon Greedy" \citep{Sutton1998e}.

What policy is most effective in this basic scenario?

Let us see how to answer this question with \pkg{contextual} by writing the following basic \proglang{R} script:
...
\hlstd{horizon}             \hlkwb{<-} \hlnum{300}
\hlcom{# Define how many times to repeat the simulation.}
\hlstd{simulations}         \hlkwb{<-} \hlnum{400}
\hlcom{# Define the probability that each ad will be clicked.}
\hlstd{click_probabilities} \hlkwb{<-} \hlkwd{c}\hlstd{(}\hlnum{0.8}\hlstd{,} \hlnum{0.2}\hlstd{)}
...
\end{kframe}
\end{knitrout}
As can be seen in the left plot in Figure \ref{fig:ctr_examples}, the Epsilon First policy is the clear winner in this scenario. Which makes sense: in contrast to the Epsilon Greedy policy, the Epsilon First policy does not incurs exploration penalties from choosing the lesser arm once it has picked the best arm.

We could, however, change this scenario just a little bit for a very different outcome. Let us, for instance, suggest that half the visitors to the web store are younger than forty and the other half older than forty---and that the store proprieter suspects one of the ads to be more effective for the younger visitors, and the other for older visitors.

We can simulate this scenario by making some minor changes to the previous script. Specifically, adapting the Bandit to generate contextual data with different reward probabilities per arm and adding a contextual policy (LinUCB, \cite{Li2010}):

\begin{knitrout}\small
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#                                    +-----+---------->  two different ads}
\hlcom{#                                    |     |}
\hlstd{click_probabilities} \hlkwb{<-} \hlkwd{matrix}\hlstd{(}  \hlkwd{c}\hlstd{(}  \hlnum{0.8}\hlstd{,}  \hlnum{0.2}\hlstd{,}    \hlcom{# -->  context: older (p=.5)}
                                    \hlnum{0.2}\hlstd{,}  \hlnum{0.8}  \hlstd{),} \hlcom{# -->  context: young (p=.5)}

                       \hlkwc{nrow} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{ncol} \hlstd{=} \hlnum{2}\hlstd{,} \hlkwc{byrow} \hlstd{=} \hlnum{TRUE}\hlstd{)}

\hlcom{# Initialize a SyntheticBandit with the *contextual* click probabilities}
\hlstd{context_bandit}      \hlkwb{<-} \hlstd{SyntheticBandit}\hlopt{$}\hlkwd{new}\hlstd{(}\hlkwc{weights} \hlstd{= click_probabilities)}
\hlcom{# Initialize a contextual LinUCBDisjointPolicy with an alpha of 0.6}
...
\hlcom{# Horizon can be much shorter here}
\hlstd{horizon}             \hlkwb{<-} \hlnum{40}
\hlcom{# Initialize a Simulator with the agent list, horizon, and number of simulations.}
\hlstd{simulator}           \hlkwb{<-} \hlstd{Simulator}\hlopt{$}\hlkwd{new}\hlstd{(agents, horizon, simulations)}
\hlcom{# Now run the simulator.}
\hlstd{history}             \hlkwb{<-} \hlstd{simulator}\hlopt{$}\hlkwd{run}\hlstd{()}
...
\begin{figure}[H]
\centering
\includegraphics[width=.99\textwidth]{fig/ctr_examples}
\label{fig:ctr_examples}
\caption{To the left, the cumulative reward rate (or click-through rate) of a non-contextual bandit scenario comparing Epsilon First and Epsilon Greedy policies---to the right the cumulative reward rate of a contextual bandit scenario comparing LinUCB, Epsilon First and Epsilon Greedy policies.}
\end{figure}

As can be seen in the right plot in Figure \ref{fig:ctr_examples}, this time around, neither non-contextual policy (Epsilon Greedy nor Epsilon First) does better than chance, as on average, both arms are equally likely to produce a click. On the other hand, the "LinUCB" policy does very well, as contextual policies are able to map rewards to contextual features.

Of course, neither simulation in the current section is very realistic. One way to generate more realistic Bandit data would be to write a Bandit subclass with a more complex generative model. How to create such a bandit subclass is delineated in section 5. Another option would, for example, be to evaluate policies on an offline data set---more on that in section 6.

\section{R6 class structure}

Since it is the \pkg{contextual}'s explicit goal to offer researchers and developers an easily extensible framework to develop, evaluate and compare their own \code{Policy} and \code{Bandit} implementations, the current section offers additional background information on \pkg{contextual}' class structure---both on the R6 class system \cite{R6} and on each of the six previously introduced core \pkg{contextual} classes.
...
Multiple agents can be combined into one plot, and multiple plots can be combined by turning off the default par formatting of the Plot class and redefining it through, for example par(mfrow, mar) and such. A script that generates some example plots that make use of these features:
...
It would, of course, be possible to evaluate policies by running them in a live setting. Such live evaluations would undoubtedly deliver unbiased, realistic estimates of a policy's effectiveness. However, the use of live data makes it more difficult to compare multiple policies at the same, as it is not possible to evaluate multiple policies at the same time with the same user. Using live data is usually also much slower than an offline evaluation, as online evaluations are dependent on active user interventions. Furthermore, the testing of policies on a live target audience, such as patients or customers, with potentially suboptimal policies, could become either dangerous or very expensive.
...
\section{Replication of LinUCB}

In the current section, we aim to demonstrate how \pkg{contextual} facilitates the comparison of bandit policies on big offline data sets by replicating the primary results of \cite{Li2010}'s seminal "A Contextual-Bandit Approach to Personalized News Article Recommendation." The paper describes how the authors ran an offline Li bandit such as the one outlined in the previous section to evaluate the effectiveness of several contextual and non-contextual policies by training and comparing them on click-through rate data drawn from the Today news module on the Yahoo! homepage over the course of several days in May 2009. With a data set of 45,811,883 separate, random and unbiased events (made available at the Yahoo! lab's website\footnote{At https://webscope.sandbox.yahoo.com/catalog.php?datatype=r\&did=49}) it is a perfect candidate for showcasing \pkg{contextual}'s ability to evaluate multiple policies in parallel.

For the details on the data structure and the general setup of the experiment, we refer the reader to \cite{Chu2009} and to the original \cite{Li2010} paper. In brief, each row in the Yahoo! dataset describes an interaction event (click or no click) of users shown a randomly chosen article. Each of these events contains information on (I) a random article chosen to be positioned at the story position on top of the Today module on Yahoo!'s website (II) six features per article for each of the articles shown below the article at the principal story position (III) six user features, where each event contains another, distinct user (IV) information on whether the user clicked on the article at the story position, or not.

More formally, for each event $t$ an article represents one of $k$ arms (that is, one of the 271 articles observed within the course of the 10 days covered by the data set) with $\mathbb{R}^6$ features $X_{t,a}$ per arm, and another $\mathbb{R}^6$ features $X_{t,u}$ per unique visitor. Together, the flattened outer product of the user and article feature vector creates a $\mathbb{R}^36$ feature vector $X_t$ for each user and article pair with the outcome value or reward $r_t$) click (1) or no click (0).

As the Yahoo data was too large to fit into memory, we imported most\footnote{The first two CSV files of the Yahoo! dataset contained irregularities, so we left those out of the import. That means that our simulations had 37,450,196 events available to them, instead of all 45,811,883. Still more than enough to replicate the original paper.} of the data set's CSV files into a MonetDB \cite{Idreos2012} instance---a very fast, open source column-oriented database management system with excellent \proglang{R} support\footnote{MonetDB can be downloaded at https://www.monetdb.org/}. The import script, example import scripts for several other databases (MySQL, SQLite, Postgresql), this replication's Bandit and Policy subclasses, and more can be found in the package's \code{demo/replication_li_2010 directory}.

With the Yahoo! data imported into the MonetDB server, the next steps are the creation of a Bandit subclass that would be able to query it, and Policy subclasses according to their description in the \cite{Li2010} paper. Since most of these Policies are already available in \pkg{contextual}, it would stand to reason that we would only have had to implement these. But as only a subset of all 271 articles or arms were shown for each separate event, we had to make some minor changes to each of these Policy classes to make them run smoothly on the constantly shifting pool of available arms. We solved this by simply adding an "arms" vector containing the indexes of all currently active arms to the context list created by the Bandit. That is, in our YahooBandit we first create a \code{self$arm_lookup} table containing all arms, and then return the following in YahooBandit's \code{get_context()}:
...
And adapted each Policy accordingly, for example, in YahooEpsilonGreedyPolicy's \code{get_action()}:
...
We were able to run seven policies (Random, EGreedy, EGreedySeg, LinUCB Dis, LinUCB Hyb, UCB1 and the UCB1Seg\footnote{EGreedy Dis and EGreedy Hyb were too summarily described in the original paper for us to be able to replicate them with confidence.}) for each of the six (0, 70, 80, 90, 95 and 99 percent) levels of sparsity defined in the original paper. This resulted in $7\times6=42$ Agents, all added to a Simulation instance and run on the full dataset. We were able to complete this simulation within 22 hours on a 64 core Intel Xeon Unbuntu server with 256GB of memory. Importantly, we decided not to split the analysis into learning and deployment buckets, as we were interested in the original policies' behavior when they would remain both exploring and exploiting over the full data set.

\begin{figure}[H]
  \centering
    \includegraphics[width=.99\textwidth]{fig/lin_99}
      \caption{A plot of cummulative reward rate (or click-through rate) for EGreedy, EGreedySeg, LinUCB Dis, LinUCB Hyb, UCB1 and the UCB1Seg policies over over the full Yahoo data set.}
      \label{fig:lin_99}
\end{figure}

From the plot, it seems that after 24 hours of training the policies had not yet settled. Which might also be concluded from our version of the original chart comparing policies' CTR's after one day of training. As can be observed in the plot below, our results did approximate the original---but not completely so. We expect this to be the result of starting our training on day three instead of day one, in combination with the longer than 24 hour period that may be needed to stabilize a comparison between all policies.

\section{Discussion and Future Work}

The current paper introduces the object-oriented \proglang{R} package \pkg{contextual}, an object-oriented and extensible framework to facilitate the parallelized comparison of contextual and non-contextual Bandit policies through simulation and offline analysis. In making the \proglang{R} package publicly available we hope to further the dissemination and evaluation of both existing and new cMAB policies. By developing the package \proglang{R}, we also hope to facilitate a lively exchange of policies between scientists and practitioners---as \proglang{R} has proven itself to be the de facto domain statistics specific computer language in both academia and industry, from the social and medical sciences to statistics and business analytics \citep{Tippmann2015}.
T



We are currently aware of a number of limitations of StreamingBandit. Firstly, as of yet,
StreamingBandit still runs single threaded. While parallelization for larger scale applications
ought to be relatively easy to implement at the level of policies, it may prove substantially
of the conditions. Also, the baseline and random conditions where manually removed after sufficient data was
collected.
Journal of Statistical Software 35
harder within policies. Yet, by forcing policies online by design, and by using state-of-the-art
web technology for its back end, StreamingBandit is already more than capable to be deployed
in a multitude of small- to medium-sized field trials. We are nevertheless of the opinion that
parallelization is an obvious next step in StreamingBandit’s development, ensuring its future
scalability.
Second, in some applications we find that certain types of rewards manifest themselves faster
than others; for example, in one instance of the use of StreamingBandit the decision to reject
a loan to a customer after an application was submitted to the firm, was much faster than
the decision (and subsequent confirmation) to accept a customer. Such an asymmetric delay
might bias a learning and thus needs to be addressed. Currently, we do not provide an off-the-
shelf solution to this problem—admittedly because it is currently unclear to us how to address
this problem in general—and hence users will need to resort to custom implementations of
the getaction and setreward code to deal with this issue.
Finally, our current cMAB libraries and toolkit still offer ample room for improvement and
extension. Outside of the currently implemented m

\section{Future Work}

Seeing the ease with which contextual can be extended to simulate most any bandit related scenario, we expect the bandit

\subsection{Doubly Robust}

\subsection{Policy Library}

\subsection{Other Bandits}
