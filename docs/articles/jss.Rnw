...
\code{ContextualBasicBandit} expects a  $d \times k$ \code{weights} matrix, where every element in \code{weights} represents the average probability of \code{ContextualBasicBandit} returning a reward of $1$ over each of its \code{k} arms as dependent on the currently active contextual feature.
...
In the next section, we introduce \code{contextual}'s implementation of \cite{Li2011}'s offline, off-policy "replay" method, which simply skips rows containing counterfactual actions \citep{Nicol2014}. In a subsequent section, we demonstrate how this "replay" method can be sensitive to bias---and how inverse propensity weighting can help remove this bias.
...
There is one element in this code that has not been introduced yet: next to \code{action$choice}, this policy also calculates the probability $p_{t,a_t}$ of selecting this action, returning it in \code{action$propensity}. This propensity can then:
