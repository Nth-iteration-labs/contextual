...
\pkg{contextual}'s, \code{Bandit} and \code{Policy} centered  class structure stays close to the previous formalization, while offering a clean, developer oriented interface. The following six classes form the backbone of the package (see also Figure ~\ref{fig:CMAB_chart}):
...
Running the \code{Simulator} instance then starts several (by default, the number of CPU cores minus one) worker processes, splitting simulations as efficiently as possible over each parallel worker. For each simulation, for every time step \code{t}, \code{agents} runs through each of the four function calls that constitute their main loop.
...
context_bandit      <- ContextualBasicBandit$new(weights = click_probs)
...
The \code{Simulator} class has been designed to make it as easy as possible to replace its default parallel backend for other \pkg{foreach} type backends. In the \code{alt_par_backend_examples} subdirectory of \pkg{contextual}'s \code{demo} directory we offer examples of \pkg{doAzureParallel}, \pkg{doRedis} and \pkg{doMPI} \code{Simulator} subclasses. As a result of \pkg{contextual}'s object oriented setup, such Simulator subclasses can be implemented in just a few lines of code, for instance in the case of the \pkg{doMPI} subclass:
...
\begin{itemize}
         ...
         \item \code{OfflineReplayEvaluatorBandit}: A basic example of a bandit that makes use of offline data as introduced by \cite{Li2010}.
\end{itemize}
...
One example of a very simple contextual Bernoulli bandit is \pkg{contextual}'s basic weight-based contextual \code{Bandit} implementation, \code{ContextualBasicBandit}:
...
\\code{ContextualBasicBandit} expects a  $d \times k$ \code{weights} matrix, where every element in \code{weights} represents the average probability of \code{ContextualBasicBandit} returning a reward of $1$, weighted over each of its \code{k} arms.
...
\caption{Top left a plot of Epsilon Greedy versus LinUCB policies' cumulative regret over time. Top middle and right, Epsilon Greedy and LinUCB policies' average arm choice percentage per $t$, which indicate that, overall, both policies choose each arm about equally often. To see why LinUCB is able to do better than Epsilon Greedy, we need to look to the bottom row, which plots LinUCB's average arm choice percentage by context feature: LinUCB makes use of the available context to map actions to rewards. We left out the "arm choice by context feature" plots for Epsilon Greedy, as all three plots would show equal allocations to each arm over every feature.}
...
In the next section, we introduce one example of such a counterfactual estimator by demonstrating \code{contextual}'s implementation of \cite{Li2011}'s "replay" method \citep{Nicol2014}. In a subsequent section, we demonstrate how this "replay" method is sensitive to bias---and how inverse propensity weighting can help remove this bias.
...
\cmidrule(r){1-3} \cmidrule(l){5-7}
 & Sport & Movie &  &  & Sport & Movie \\ \cmidrule(r){1-3} \cmidrule(l){5-7}
\begin{tabular}[c]{@{}l@{}}Male\\ Female\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.4 x 0.5\\ 0.8 x 0.5\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.3 x 0.5\\ 0.7 x 0.5\end{tabular} &  & \begin{tabular}[c]{@{}l@{}}Male\\ Female\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.4 x 0.75\\ 0.8 x 0.25\end{tabular} & \begin{tabular}[c]{@{}l@{}}0.3 x 0.25\\ 0.7 x 0.75\end{tabular} \\ \cmidrule(r){1-3} \cmidrule(l){5-7}
Per category CTR & 0.6 & 0.5 &  & Per category CTR & 0.5 & 0.6 \\ \cmidrule(r){1-3} \cmidrule(l){5-7}
\end{tabular}
\caption{To the left, a table with unbiased per category click-through rates for male and female visitors to a website. Here, visitors are randomly assigned to article categories. For example, male visitors click-through to sports articles four out of ten times, and three out of ten times to movie articles---displaying each type of article 50\% of the time. To the right, a table representing the same site with the same visitors where the assignement of visitor types to categories is skewed. For instance, here, male visitors are shown sports type articles 75\% and movie articles 25\% of the time, and female visitors vice versa. }
\label{table:bias}
\end{table}
Clearly, both male and female visitors prefer sports over movie related articles, which is reflected by the overall CTR per article category. A scenario that can be converted to a \pkg{contextual} bandit scenario with much difficulty, with article categories for arms, and male and female visitors for context:
  ...
  public = list(
    ...
    get_action = function(t, context) {
      if(context$X[1,1]==1) {  # 1: Male, 0: Female.
        prob <- c(0.75,0.25)   # Editor thinks men like sports
      } else {
        prob <- c(0.25,0.75)   # Editor thinks women like movies
      }
...
There is one element in this code that has not been introduced yet: next to \code{action$choice}, this policy also calculates the probability $p_t$ of selecting this action, returning it in \code{action$propensity}. This propensity can then:
...
Simpson's Paradox all over again. Yet when we make one minor change to our original replay evaluator, implementing $ips$ though the multiplaction of rewards by \code{1 / action$propensity} (see \pkg{contextual}'s  \code{OfflineReplayEvaluatorBandit} for the full implementation), we are able to correct for the editor's bias:
