...
\code{Plot} objects can be instantiated directly, or, more commonly, by calling the \code{plot()} function. In either case, make sure to specify (at least) a \code{History} instance and one of the plot types specified above:
