...
In the current paper, we will further introduce \pkg{contextual}. Section 2 presents a formal definition of the contextual multi-armed bandit problem, shows how this formalization can be transformed into a clear and concise object-oriented architecture, and describes how to set up a minimal simulation. Section 3 gives an overview of \pkg{contextual}'s predefined \code{Bandit} and \code{Policy} subclasses and demonstrates how to run a very basic \code{Simulation}. Section 4 delves a little deeper into the implementation of each of pkg{contextual}'s core superclasses. Section 5 shows how to extend \pkg{contextual}'s superclasses to create your own custom \code{Bandit} and \code{Policy} subclasses. Section 6 demonstrates how to further subclass existing Bandit and Policy implementations. Section 7 focusses on how a \code{Bandit} subclass can make use of offline datasets. Section 8 brings all of the previous sections together in a partial replication of a frequently cited contextual bandit paper. We conclude with some comments on the current state of the package and potential future enhancements.

This organization offers the reader several ways to peruse the current paper. If you have a passing knowledge of \proglang{R} and want to run simulations based on \pkg{contextual}'s default bandits and policies, the current introduction plus section 3 should be able to get you up and running. If you are looking for a more formal introduction, read section 2 as well. If you know your way around \proglang{R} and would like to extend \pkg{contextual} to run custom bandits and policies, it is probably best to read the whole paper---with a focus on sections 4, 5 and 6. Finally, add sections 7 and possibly 8 if you are seeking to implement your own offline bandits.
...
\begin{itemize}
         ...
         \item \code{Simulator}: R6 class \code{Simulator} is the entry point of any \pkg{contextual} simulation. It encapsulates one or more \code{Agents} (running in parallel to each other, by default), creates an Agent clone (each with its own deterministic seed) for each to be repeated simulation, runs the \code{Agents}, and saves the log of all \code{Agent's} interactions to a \code{History} object.
...
In these lines we start out by instantiating \code{Policy} subclass \code{EpsilonGreedyPolicy} (covered in section \ref{epsgreedy}) as object \code{policy}, with its \code{epsilon} parameter set to \code{0.1}. Next, we instantiate the \code{Bandit} subclass \code{BasicBernoulliBandit} (covered in section \ref{extending}) as \code{bandit}, with three Bernoulli arms, each offering a reward of one with reward probability $\theta$, and otherwise a reward of zero. For the current simulation, we have set the \code{bandit} amr probabilities of reward to respectively 0.9, 0.1 and 0.1 through the policy's \code{weight} parameter. We then assign both our \code{bandit} and our \code{policy} to \code{Agent} instance \code{agent}. This \code{agent} is  added to a \code{Simulator} that is set to one hundred \code{simulations}, each with a \code{horizon} of fifty---that is, \code{simulator} runs one hundred \code{simulations}, each with a different (deterministic) random seed, for fifty time steps \code{t}.

Running the \code{Simulator} starts several (by default, the number of CPU cores minus one) worker processes, splitting simulations as efficiently as possible over each parallel worker. For each simulation, for every time step \code{t}, \code{agents} runs through each of the four function calls that constitute their main loop.
...
The current section offers additional background information on \pkg{contextual}'s class structure---both on the R6 class system \cite{R6} and on each of the six previously introduced core \pkg{contextual} classes. Together with the information in the next section, on bandit and policy implementation, this should be able to get you up and running with developing your own custom bandits and pol
...
To specify how to run a simulation and which data is to be saved to a \code{Simulator} instance's \code{History} log, a \code{Policy} object can be configured through, amongst others, the following arguments:
...
The current section illustrates how to develop custom Bandit and Policy subclasses through an exploration of \pkg{contextual}'s Bernoulli bandit and its EpsilonFirst, EspilonGreedy, and LinUCB policy implementations.
...
To evaluate this policy, instantiate both an \code{EpsilonFirstPolicy} and a \code{ContextualBernoulliBandit} (a contextual and more versatile \code{BasicBernoulliBandit} subclass). Then add the \code{Bandit}/\code{Policy} pair to an \code{Agent}. Next, add the \code{Agent} to a \code{Simulator}. Finally, run the \code{Simulator}, and \code{plot()} the its \code{History} log:
...
\end{Code}
\includegraphics[width=\textwidth]{fig/section_5_3}
...
lucb_policy <- EpsilonGreedyPolicy$new(0.1)
...
\end{Code}
\includegraphics[width=\textwidth]{fig/section_5_4}
...
plot(history, type = "arms",  limit_agents = c("EG"), no_par = TRUE,
              interval = 25)
\end{Code}
\includegraphics[width=\textwidth]{fig/section_5_5}
...
url <- paste0(url,"Nth-iteration-labs/contextual_data/master/")
url <- paste0(url,"data_persuasion_api/persuasion_api_daypart.csv")
...
history     <- Simulator$new(agents, horizon, sims, reindex = T)$run()

plot(history, type = "cumulative", regret = FALSE, smooth = TRUE,
     traces = TRUE, rate = TRUE, ylim = c(0.0105, 0.014), legend = FALSE)
\end{Code}

See Figure \ref{fig:section_8_bar} for a plot of the resulting click-through rate over time.
...
The policy classes then use this information to select and update only the currently active subset of arms. For instance, in YahooEpsilonGreedyPolicy's \code{get_action()}:
