...
Though \pkg{contextual} really comes into its own when you start building your own bandit and policy classes, you can also use one of the packages build-in policies, and run them on one of \pkg{contextual}'s synthetic or offline bandits (see Table \ref{table:overview_policies} for an overview of available policies).

\begin{table}[H]
\begin{tabular}{@{}llllll@{}}
\toprule
     & e-Greedy                                                   & Upper Confidence Bound                                   & Thomspon Sampling                                                       & Other                                                     & Special                                                                        \\ \midrule
MAB  & \begin{tabular}[c]{@{}l@{}}e-Greedy\\ e-First\end{tabular} & \begin{tabular}[c]{@{}l@{}}UCB1\\ UCB-tuned\end{tabular} & \begin{tabular}[c]{@{}l@{}}Thompson Sampling\\ BootstrapTS\end{tabular} & \begin{tabular}[c]{@{}l@{}}Softmax\\ Gittins\end{tabular} & \multirow{2}{*}{\begin{tabular}[c]{@{}l@{}}Random\\ Oracle\\ LiF\end{tabular}} \\ \cmidrule(r){1-5}
cMAB & Epoch-Greedy                                               & \begin{tabular}[c]{@{}l@{}}LinUCB\\ COFIBA\end{tabular}  & LinTS                                                                   &                                                           &                                                                                \\ \bottomrule
\end{tabular}
\caption{An overview of \pkg{contextual}'s predefined policy classes.}
\label{table:overview_policies}
\end{table}

You may, for instance, want to find out which bandit policy is the most useful for selecting the best out of two new online adds for your new web store.

One option would be to show both adds at random (explore) for a set period of time and thereafter go with the add that has the best Click-Through Rate (exploit). This policy is known as an epsilon-first policy or, more commonly, as an A/B test (or RCT, for that matter). Another option might be to randomly show one of the adds 10 percent of the time (explore), and show the best add 90 percent of the time (exploit). This type of policy is known as an "epsilon-greedy" policy.

So the epsilon-greedy policy can keep on exploring over time. On the other hand, the epsilon-first policy starts out with a full-time exploration. So which is better? And under what conditions?
