...
It's main function is \code{do_step()}, generally called by the worker of a \code{Simulator} object that takes care of the running of a particular \code{agent} instance:
...
To specify the paramaters of a simulation and the data saved to the history log, Simulator takes a multitude of arguments to fine tune a Simulator's instantiation. Some of the more important are the following:
...
Though section 3 gives an overview of all six of contextual's root classes, in practice most developers and researchers will mostly focus on the implementation or subclassing of their own policies and bandits. Therefore, the current section first demonstrates how to  implement some well known bandit algorithms. Next we show how to create a Policy sub-subclass that inherits from one of the previously defined policy's, togehter with a very basic Bandit sub-subclass. Finally, we'll demonstrate

previously imple together with a very basic custom polocy,  policy pair
...
At that point, it's easy to run any policy by assigning it to an agent together with one of contextual's Bandit subclasses. Then assign and run the agent via a Simulator instance:
...
First the Epsilon Greedy algorythm in pseudocode:
...
As a final example of how to subclass contextual's Bandit superclass, we show how to implement one of the most often cited contextual bandit policies, the LinUCB with Linear Disjoint Models algorithm. First, again, a high-level description the LinUCB Disjoint algorithm in pseudocode:
...
Of course, the extensibility of contextual does not limit itself to the subclassing of Policies. Through its R6 based object system, it's also easy to extend any other \pkg{contextual} super-  or subclass. Below, we demonstrate how to make use of that extensibility through the implementation of a PoissonRewardBandit extending \pkg{contextual}'s BasicBandit class, and of an PoissonRewardBandit version of the Epsilon Greedy policy that we introduced in section 4.2.
...
Though section 5 focussed on the implementation of custom Policy subclasses and a very basic custom non-contextual Bandit class, one of the strengths of the contextual package is its flexibility in the evaluation and comparison of offline policies.

\subsection{Offline evaluation through LiLogBandit}

Though it is, as demonstrated in the previous section, relatively easy to create basic synthetic Bandits to test simple MAB and cMAB policies, the creation of more complex simulations that generate more complex contexts for more demanding policies can become very complicated very fast. So much so, that the implementation of such simulators regularly becomes more complex than the analysis and implementation of the policies themselves. And even when you succeeed in surpassing these technical challenges, it remains an open question if an evaluation based on simulated data reflects real-world applications--as modeling by definition introduces bias.
