...
It is also possible to retrieve a data.frame object from an History object, and plot the data in any other way, for instance with ggplot (both with 95% confidence interval):

<<eval=TRUE, echo=FALSE, include = FALSE>>=
library("contextual") # run contextual simulation & plot results (slightly faster)
library("ggplot2")    # followed by the some plot through ggplot (more versatile)

@
<<label=ggplotcompare, size='small', message = FALSE, fig.width=10, fig.height=3, fig.keep='high'>>=
...
plot(history, type = "average", regret = FALSE, ci = TRUE, no_internal_par = TRUE)
...
Following the instantiation of the Agent Bandit/Policy pair it can now be assigned to, and then run on, a Simulator:

<<label=efplot, size='small',  message = FALSE, fig.keep='none'>>=
horizon            <- 100
simulations        <- 100
...
plot(history, type = "arms", no_internal_par = TRUE)
@
\begin{center}
<<label=efpolicy, fig.width=10, fig.height=3.7, fig.keep='high', echo=FALSE, message=FALSE, out.extra='', fig.cap="Epsilon First">>=
<<efplot>>
@
\end{center}

Importantly, where not otherwise noted, the SyntheticBandit in the code above, and all other Bandits in the current paper refer to (or will be configured as) Bernoulli (or "Binary") bandits: that is, on each $t$ they offer either a reward of $1$ with probability $p$ or a reward of $0$ with probability $p-1$.
...
Assign the new class plus our Bernoulli type Bandit to an Agent, assign the Agent to a Simulator, and run it:

<<label=egplot, size='small',  message = FALSE, fig.keep='none'>>=
horizon            <- 100
simulations        <- 100
weights            <- c(0.9, 0.1, 0.1)
...
@
\begin{center}
<<label=egpolicy, fig.width=10, fig.height=3.7, fig.keep='high', echo=FALSE, message=FALSE, out.extra='', fig.cap="Epsilon Greedy">>=
<<egplot>>
@
\end{center}
...
Next, translate the above pseudocode into a well organized Bandit subclass again:
...
Let us now evaluate this Policy on a synthetic Bernoulli Bandit with three arms and three context features. As you can observe in the code below, each of the synthetic Bandit's arms is on average equally probable to return a reward. At the same time, the (randomized) presence of a context feature has a strong influence on the distribution of rewards over the arms: in the presence of a specific feature, one of the arms is much more likely to offer a reward. In this setting, the EpsilonGreedyPolicy does not do better than chance. But the LinUCBDisjointPolicy is able to learn the relationships between arms, rewards, and features without much difficulty:

<<label=lucbplot, size='small',  message = FALSE, fig.keep='none'>>=
...
Of course, the extensibility of contextual does not limit itself to the subclassing of Policy classes. Through its R6 based object system, it is also easy to extend and override any other \pkg{contextual} super-  or subclass. Below, we demonstrate how to make use of that extensibility through the implementation of a PoissonRewardBandit extending \pkg{contextual}'s BasicBandit class, and of an EpsilonGreedyAnnealingPolicy version of the Epsilon Greedy policy that we introduced in section 4.2.

<<label=poisson, size='small',  message = FALSE, fig.keep='none'>>=
...
@
\begin{center}
<<label=poissonplot, fig.width=10, fig.height=3.7, fig.keep='high', echo=FALSE, message=FALSE, out.extra='', fig.cap="Extending BasicBandit and EpsilonGreedyPolicy">>=
<<poisson>>
@
\end{center}
...
url <- "https://raw.githubusercontent.com/Nth-iteration-labs/
        contextual_data/master/PersuasionAPI/persuasion.csv"
...
par(mfrow = c(1, 2), mar = c(2,4,1,1))
plot(history, type = "cumulative", regret = FALSE, rate = TRUE, ylim = c(.0105, .015))
plot(history, type = "arms")
@
...
\begin{figure}[H]
  \centering
    ...
    \label{fig:ucb_offline}
      \caption{An UCB1 based bandit evaluated on a Li Bandit. The Bandit itself samples from 350000 rows with clicks for rewards and the display of one of four “persuasive strategies” to users of an online store representing the offline Bandit's four arms.}
\end{figure}
...
Thanks go to my colleagues at the JADS...
